...
## Adaptation for CoThe.Info

The viewer code on `/atlas` is ported from [ashemag/human-atlas](https://github.com/ashemag/human-atlas)
(MIT License) and re-themed for CoThe.Info: dark palette, Vietnamese/English
interface, and a Vietnamese glossary layered over the English source names.

Only the gzipped geometry chunks are shipped (`body-*.bin.gz`, ~32 MB); the
manifest `atlas.json` points at them directly. Geometry, concept names and
system groupings are unchanged from the human-atlas build, so the BodyParts3D
CC BY 4.0 attribution above applies unchanged.

---

Trình xem 3D ở trang `/atlas` được port từ [ashemag/human-atlas](https://github.com/ashemag/human-atlas)
(giấy phép MIT) và đổi giao diện cho CoThe.Info. Dữ liệu giải phẫu vẫn là
BodyParts3D 4.0 theo giấy phép CC BY 4.0 — khi phân phối lại phải giữ nguyên
phần ghi công ở trên.
